...
**✅ Remote access LIVE** (2026-07-11): `https://marqu33.duckdns.org` → the server, confirmed
from outside the network. Path: **UniFi UDM Pro** forwards **443** → the Dell (192.168.1.103);
**Caddy** (`C:\caddy`, with the `caddy-dns/duckdns` plugin) auto-TLS via **DNS-01**; **PIA
split-tunnels `caddy.exe`** (which also fixed PIA's port-53 DNS-leak block → resolvers `1.1.1.1`).
Invite code on the Dell: `lantern-6274`.
...
1. **Verify the TV app on the real TCL** — D-pad nav (arrows/Enter → focus engine), the Back
   button, and video playback. If arrows don't drive it, translate D-pad → arrow keydowns in
   `MainActivity`.
2. **Auto-start on Dell boot** — media server + Caddy relaunch after Windows-update reboots
   (auto-login + Task Scheduler at logon; NVENC needs an interactive session). Prompt relayed.
3. **TV app v2** — release signing (consistent keystore) + an in-app self-updater (check
   `marquee-tv-latest`, download, install) so updates skip re-sideloading; plus a server-URL setting.
